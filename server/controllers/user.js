...
export const register = tryCatch(async (req, res) => {
    ...
    if (password.length < 6) return res.status(400).json({ success: false, message: 'Password must be 6 or more' });
...
export const updateProfile = tryCatch(async (req, res) => {
    ...
    const token = jwt.sign({ id, name, photoURL }, process.env.JWT_SECRET, { expiresIn: '1h' });
    res.status(200).json({ success: true, result: { id, name, photoURL, token } });
});
